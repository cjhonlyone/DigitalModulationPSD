function y = amiqmod(x, Fc, Fs, varargin)
...
%   default carrier amplitude is zero, so the function implements suppressed
%   carrier modulation.
%
...
%
%   Fs must be greater than 2*Fc. Furthermore, to avoid aliasing, Fs must
%   be greater than twice the sum of Fc and the input signal bandwidth.
%
...
y = (x + carr_amp)./(max(x)+carr_amp) .* exp(1i*2*pi*t*Fc + ini_phase);
